...
def greet():
    ...
    conn.commit()
    conn.close()
    SR.speak("\nMyself Rachel How may I help you?")
...
def mainframe():
    ...
    try:
        while(True):
            ...
            #wikipedia search
            if there_exists(['find','search','search wikipedia for','from wikipedia'],query):
            ...
            #asking for name
                ...
                SR.speak("My name is Rachel, You can call me Rach and I'm here to serve you.")
            #How are you
            ...
            #playing on youtube
                ...
                break
            elif there_exists(['play some songs on youtube','i would like to listen some music','i would like to listen some songs','play songs on youtube','music','songs'],query):
                SR.speak("Opening youtube")
                ...
                break
            elif there_exists(['open google and search','google and search','search','google'],query):
            ...
            #image search
                ...
                url="https://www.google.com/search?q="+query[query.find('of')+3:]
                webbrowser.get().open(url)
                break
            ...
            #play music
                ...
                SR.speak("Sorry You don't have any music downloaded")
                music_dir=''
                continue
            ...
            #time and date
            elif there_exists(['time'],query):
                strTime =datetime.datetime.now().strftime("%H:%M:%S")
                SR.speak(f"Sir, the time is {strTime}")
            elif there_exists(['date'],query):
            ...
            #opening software applications
                ...
                codepath=r"C:\Users\Rohit Gupta\AppData\Local\Programs\Microsoft VS Code\Code.exe"
            ...
            #not working
                ...
                os.startfile(r'C:\Users\Rohit Gupta\AppData\Local\WhatsApp\WhatsApp.exe')
                
            elif there_exists(['control panel','open control panel','open this computer setting Window','open computer setting Window'   ,'open computer settings','open setting','show me settings','open my computer settings'],query):
            ...
            #weather report
            elif there_exists(['weather','temperature'],query):
            ...
            #what is the capital
                ...
                try:
                    ...
                    print("Internet Connection Error")
            #fitbit api
            # elif ('today' and ('steps')) in query or ('weekly' and ('step' or 'steps')):
            #     SR.speak(f"Hey you made {fit.steps:.2f} steps today {fit.make_suggestions(fit.calories,fit.steps, fit.distance, 'today')}")
            # elif ('today' and ('calories')) in query:
            #     SR.speak(f"Hey you burnt {fit.calories:.2f} calories today {fit.make_suggestions(fit.calories,fit.steps, fit.distance, 'today')}")
            # elif ('today' and ("distance")) in query:
            #     SR.speak(f"Hey you walked {fit.distance:.2f} distance today{fit.make_suggestions(fit.calories,fit.steps, fit.distance, 'today')}")

            # elif ('week' and ('step' or 'steps')) in query or ('weekly' and ('step' or 'steps')) in query:
            #     SR.speak(f"Hey you made {fit.week_steps:.2f} steps this week {fit.make_suggestions(fit.week_calories,fit.week_steps, fit.week_distance, 'weekly')}")
                
            # elif ('month' and ('step' or 'steps')) in query or ('monthly' and ('step' or 'steps')) in query:
            #     SR.speak(f"Hey you made {fit.month_steps:.2f} steps this week {fit.make_suggestions(fit.month_calories,fit.month_steps, fit.month_distance, 'weekly')}")
            
            # elif ('year' and ('step' or 'steps')) in query or ('yearly' and ('step' or 'steps')) in query:
            #     SR.speak(f"Hey you made {fit.year_steps:.2f} steps this week {fit.make_suggestions(fit.year_calories,fit.year_steps, fit.year_distance, 'weekly')}")

            # elif ('week' and ('calories')) in query or ('weekly' and ('calories')) in query:
            #     SR.speak(f"Hey you burnt {fit.week_calories:.2f} calories this week {fit.make_suggestions(fit.week_calories,fit.week_steps, fit.week_distance, 'monthly')}")
               
            # elif ('month' and ('calories')) in query or ('monthly' and ('calories')) in query:
            #     SR.speak(f"Hey you burnt {fit.month_calories:.2f} calories this week {fit.make_suggestions(fit.month_calories,fit.month_steps, fit.month_distance, 'monthly')}")
            # elif ('year' and ('calories')) in query or ('yearly' and ('calories')) in query:
            #     SR.speak(f"Hey you burnt {fit.year_calories:.2f} calories this week {fit.make_suggestions(fit.year_calories,fit.year_steps, fit.year_distance, 'monthly')}")
                

            elif ('week' and 'distance') in query or ('weekly' and ('distance')) in query:
                SR.speak(f"Hey you burnt {fit.week_distance:.2f} distance this week {fit.make_suggestions(fit.week_calories,fit.week_steps, fit.week_distance, 'yearly')}")
                
            elif ('month' and ('distance')) in query or ('monthly' and ('distance')) in query:
                SR.speak(f"Hey you burnt {fit.month_distance:.2f} distance this week {fit.make_suggestions(fit.month_calories,fit.month_steps, fit.month_distance, 'yearly')}")
            
            elif ('year' and ('distance')) in query or ('yearly' and ('distance')) in query:
                SR.speak(f"Hey you burnt {fit.year_distance:.2f} distance this week {fit.make_suggestions(fit.year_calories,fit.year_steps, fit.year_distance, 'yearly')}")
            else:
                SR.speak("Sorry, I have no idea about your request "+query)
        SR.speak("See you soon ! Goodbye Take Care")
    except Exception as e:
        pass
...
if __name__=="__main__":
        ...
        root.geometry("{}x{}+{}+{}".format(1000,500,int(root.winfo_screenwidth()/2 - 1000/2),int(root.winfo_screenheight()/2 - 500/2)))
        root.resizable(0,0)
        root.title("Heisenberg")
        root.iconbitmap('Heisenberg.ico')
        root.configure(bg='#2c4557')
        scrollable_text=scrolledtext.ScrolledText(root,state='disabled',height=22,width=100,relief='sunken',bd=5,wrap=tk.WORD,bg='#add8e6',fg='#800000')
        scrollable_text.place(x=10,y=10)
        mic_img=Image.open("Mic.png")
        mic_img=mic_img.resize((70,70),Image.ANTIALIAS)
        ...
        Listen_Button.place(x=400,y=350)
        myMenu=tk.Menu(root)
